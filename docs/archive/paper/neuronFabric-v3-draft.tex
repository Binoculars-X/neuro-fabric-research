...
Shakespeare corpus. The BF16W variant reaches eval loss 1.5477 at 80K samples
(GPU FP32 oracle: 1.5226), with coherent character-level text generation confirmed.
...
\textbf{Code:} \href{\repourl}{\repourltext} (release v1.0.3, commit \texttt{97c09f3}).
...
\begin{enumerate}
  ...
  \item \textbf{BF16W} --- a mixed-precision scheme where weights are stored as BF16
    ...
    convergence penalty ($+0.025$ val loss) on Shakespeare.
...
reaches eval loss 1.5477 versus GPU FP32 1.5226 --- a gap of $+0.025$.
...
All experiments were conducted using NeuronFabric release \textbf{v1.0.3}.
...
Training: 80,000 samples (online, batch$=1$), GPU accelerated.
...
\midrule
GPU Adam FP32  & 80,000 & \textbf{1.5226} & 55.79\% & 19.37 \\
CPU Adam BF16W & 80,000 & 1.5477 & 54.57\% & 149.51 \\
\bottomrule
\end{tabular}
\caption{Shakespeare 334K training results (NeuronFabric v1.0.3, run/results/).
GPU Adam FP32 is the oracle baseline.
CPU BF16W gap: $+0.025$ val loss, $7.7\times$ slower --- consistent with the precision difference between BF16 and FP32 weight storage.}
\label{tab:shakespeare-results}
\end{table}

\textbf{Training curve.} Best eval loss 1.5226 (GPU FP32, at 80K) and 1.5477 (CPU BF16W, at 78K)
...
BF16W model trained for $\sim$3.3 hours on CPU (no GPU required).
...
GPU Adam FP32 reaches eval loss 1.5226 at 80K samples (oracle baseline).
CPU Adam BF16W reaches 1.5477.}
...
\textbf{Projected training time on ZCU102 (unverified).} At 150--200\,MHz (a realistic target for a transformer core with control logic and interconnects) with $\sim$600 BF16 FMA units, fully pipelined: $\sim$15--30\,ms/sample. Power: $\sim$15\,W.
...
\begin{itemize}
  ...
  \item BF16W Adam converges with a negligible gap ($+0.025$ val loss) versus FP32 Adam on Shakespeare at 80K samples.
...
eval loss 1.5477 --- with 660\,KB of on-chip SRAM headroom to spare on a mid-range FPGA.
