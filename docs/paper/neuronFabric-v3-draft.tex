...
Shakespeare corpus. It reaches eval loss 1.5226 at 80K samples using GPU Adam,
...
\textbf{Code:} \href{\repourl}{\repourltext} (release v1.0.2, commit \texttt{86642e2}).
...
All experiments were conducted using NeuronFabric release \textbf{v1.0.2}.
...
\bottomrule
\end{tabular}
\caption{Shakespeare 334K training results (NeuronFabric v1.0.2, run/results/).
...
\caption{Validation loss vs training samples for Shakespeare 334K model (NeuronFabric v1.0.2).
GPU Adam FP32 reaches eval loss 1.5226 at 80K samples (oracle baseline).
CPU Adam BF16W reaches 1.5477 --- a gap of $+0.025$, consistent with the precision difference between BF16 and FP32 weight storage.}
...
BF16W model trained for 11,961 seconds on CPU (no GPU required).
...
Code repository: \href{\repourl}{\repourltext}, release tag \texttt{v1.0.2}.
