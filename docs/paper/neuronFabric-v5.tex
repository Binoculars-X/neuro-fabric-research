...
\newcommand{\bfsixteenloss}{0.025}
\newcommand{\gpuvaloss}{1.5224}
\newcommand{\cpubfvaloss}{1.5477}
...
perform the weight update off-chip. The weight update does not happen on the chip
...
Shakespeare corpus. The BF16W variant reaches eval loss \cpubfvaloss{} at 80K samples
...
The chip that did the work does not do the learning.
...
GPU Adam FP32  & 80,000 & \textbf{\gpuvaloss} & 2.20 & 55.66\% & 17.13 \\
    CPU Adam BF16W & 80,000 & \cpubfvaloss & 2.23 & 54.57\% & 149.51 \\
\midrule
Karpathy char-rnn~\cite{karpathy2015charrnn} & --- & --- & $\sim$1.3 & --- & --- \\
\bottomrule
\end{tabular}
\caption{Shakespeare 334K training results (NeuronFabric \repoversion, run/results/).
GPU Adam FP32 is the oracle baseline.
CPU BF16W gap: $+\bfsixteenloss$ val loss, $7.7\times$ slower --- consistent with the precision difference between BF16 and FP32 weight storage.
...
\textbf{Training curve.} Best eval loss \gpuvaloss{} (GPU FP32, at 80K) and \cpubfvaloss{} (CPU BF16W, at 78K)
...
\textbf{2.21 BPC} (bits per character, = eval loss / $\ln 2$ = $\cpubfvaloss / 0.6931$),
